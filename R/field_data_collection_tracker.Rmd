...
output: 
  html_document:
    ...
    toc_float:
      collapsed: false
      smooth_scroll: false
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = paste0(dirname(inputFile), '/', butteR::date_file_prefix(),'_field_data_collection_tracker.html')) })
---
...
>There are **`r nrow(df_tool_data)`** total number of surveys done as of **`r df_days_for_data_collection[length(df_days_for_data_collection)]`**.

### Settlements:  **`r df_tool_data %>% filter(status == "refugee") %>% nrow()`** surveys
...
### Host community: **`r df_tool_data %>% filter(status == "host_community") %>% nrow()`** surveys
